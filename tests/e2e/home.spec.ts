...
test("scan form submits and renders no-findings result", async ({ page }) => {
	...
	await page.getByLabel("Domain target").fill("localhost:4173/sandbox/website/no-leak");
...
test("scan form submits and renders redacted finding", async ({ page }) => {
	...
	await page.getByLabel("Domain target").fill("localhost:4173/sandbox/website/pem-key");
	await page.getByRole("button", { name: "Run scan" }).click();
